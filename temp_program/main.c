...
int main(void) {
    const Level easy = { .kMaxNum = 100, .kMaxCnt = 15 },
                itrm = { .kMaxNum = 150, .kMaxCnt = 13 },
                hard = { .kMaxNum = 200, .kMaxCnt = 11 };
    ...
    while (true) {
        ...
        puts((NULL ||
            level == '1' || level == 'E' ? playUpDownGame(&easy) :
            level == '2' || level == 'I' ? playUpDownGame(&itrm) :
            level == '3' || level == 'H' ? playUpDownGame(&hard) :
            false) 
